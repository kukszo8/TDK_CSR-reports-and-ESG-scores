...
```{r}
pin_read(board, "k_result_stm-time") |> 
  select(
    K,
    `Lower bound`,
    Residuals,
...
##Proba
esg_sparse<-pin_read(board,"esg_sparse")
stm_time_12<-pin_read(board,"stm-time_12") %>% 
.[[1]]

td_beta <- tidy(stm_time_12)


td_gamma <- tidy(stm_time_12, matrix = "gamma",
                 document_names = rownames(esg_sparse))
